  ...
  d3.layout.phylotree = function(container) {
    ...
    var self = {},
      ...
      options = {
        ...
        compression: 0.2,
        "align-tips": false,
        "maximum-per-node-spacing": 50,
        "minimum-per-node-spacing": 2,
        "maximum-per-level-spacing": 40,
        ...
        if (node.parent) {
            /*      if(node.name.length != 0) {
                      menu_object
                          ...
                          .on("click", function (d) {
                              $.ajax({
                                  url: "/fern/rna/checkName?name=" + node.name,
                                  type: "get",
                                  dataType: "json",
                                  success: function (data) {
                                      if (data.valid == "false") {
                                          ...
                                          window.open("/fern/rna/moreInfoByName?name=" + node.name);
                                      }
                                  }
                              });
...
/**
 * Determine whether a given node is a leaf node.确定给定的节点是否是叶子节点
 *
...
/**
 * Get nodes which are currently selected.获取当前选中的节点。
 *
...
/**
 * Reroot the tree on the given node.对给定节点上的树进行Reroot。
 *
...
/**
 * Update a given key name in each node.更新每个节点中给定的密钥名。
 *
...
/**
 * Get or set spacing in the x-direction.获取或设置x方向的间隔。
 *
...
/**
 * Get or set spacing in the y-direction.获取或设置y方向的间隔。
 *
 * @param {Number} attr (Optional), the new spacing value if setting.
 * @param {Boolean} skip_render (Optional), whether or not a refresh should be performed.
...
/**
 * Get or set branch length accessor.获取或设置分支长度访问器。
 *
...
/**
 * Get or set branch name accessor.获取或设置分支名称访问器。
 *
...
/**
 * Lay out the tree within the SVG.在SVG中布局树。
 *
 ...
 * terminal/internal nodes.选择给定节点的所有后代，并提供选择选项终端/内部节点。
 *
    ...
    phylotree.draw_node = function(container, node, transitions) {
      ...
      if (d3_phylotree_is_leafnode(node)) {
        var labels = container.selectAll("text").data([node]),
          tracers = container.selectAll("line");//last_one
        if (transitions) {
            labels
            .enter()
            .append("text")
...
/**
 * Get an array of all nodes.获取所有节点的数组。
 *
...
/**
 * Determine if a given node is a leaf node.确定给定的节点是否是叶子节点。
 *
  ...
  function d3_phylotree_newick_parser(nwk_str, bootstrap_values) {
    ...
    function finish_node_definition() {
      ...
      if (bootstrap_values && "children" in this_node) {
        ...
        this_node["name"] = "<a class='openInfo' onclick='openInfo(this)' id='"+current_node_name +"'>&nbsp; * &nbsp;</a>" + current_node_name  ;
      }
      this_node["attribute"] = current_node_attribute; //虚线便实现，注释便可
