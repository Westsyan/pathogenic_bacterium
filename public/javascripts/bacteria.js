...
var array = ["目（order）","亚目（Suborder）","科（family）","属（genus）","种（species）",
...
var tax =  ["目（order）", "亚目（Suborder）", "科（family）", "属（genus）", "种（species）"];
...
if(String(href).indexOf("/admin/") != -1) {
    $("#table").bootstrapTable({
        columns: [{
            ...
            title:"名称",
            formatter:function (value,row,index) {
                var de = "<i><a href='/english/Becteria/moreInfo?id="+ row.id+"' target='_blank'>" + row.name +"</a></i>";
                return de;
            }
        }]
    });
