...
class IndexController @Inject()(passworddao: passwordDao) extends Controller {
  ...
  def logout = Action{implicit request=>
    Redirect(routes.IndexController.home()).withNewSession
  }
  ...
  def login = Action.async{implicit request=>
    ...
    passworddao.selectByAccount(account).map{x=>
      if(x.password == pwd){
        Redirect(routes.IndexController.home()).withSession(request.session + ("id" -> x.id.toString)+("user" -> x.user))
      }else{
        Redirect(routes.IndexController.toLogin()).flashing("info" -> "账号密码错误，请重新输入！")
      }
    }
  }
