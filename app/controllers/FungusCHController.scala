...
class FungusCHController @Inject()(fungusdao: fungusDao) extends Controller  {


  def browse = Action {implicit request=>
    Ok(views.html.chinese.fungus.browse())
  }
  ...
  def moreInfo(id:Int) = Action.async{implicit request=>
    fungusdao.getById(id).map{x=>
      Ok(views.html.chinese.fungus.moreInfo(transForm.transFungusRow(x)))
    }
  }

  def searchByTaxBefore = Action{implicit request=>
    Ok(views.html.chinese.fungus.searchByTax())
  }
  ...
  def uploadBefore = Action{implicit request=>
    Ok(views.html.chinese.fungus.uploadFungus())
  }
  ...
  def updateBefore(id:Int) = Action.async{implicit request=>
    fungusdao.getById(id).map{x=>
      Ok(views.html.chinese.fungus.updateFungus(transForm.transFungusRow(x)))
    }
  }
  ...
  def manageBefore = Action{implicit request=>
    Ok(views.html.chinese.fungus.manageFungus())
  }
