...
class VirusCHController @Inject()(virusdao: virusDao) extends Controller {
  ...
  def browse = Action {implicit request=>
    Ok(views.html.chinese.virus.browse())
  }
  ...
  def searchByTaxBefore = Action {implicit request=>
    Ok(views.html.chinese.virus.searchByTax())
  }
  ...
  def moreInfo(id: Int) = Action.async {implicit request=>
    virusdao.getById(id).map { x =>
      Ok(views.html.chinese.virus.moreInfo(transForm.transVirusRow(x)))
    }
  }
  ...
  def uploadBefore = Action{implicit request=>
    Ok(views.html.chinese.virus.uploadVirus())
  }
  ...
  def manageBefore = Action { implicit request =>
    Ok(views.html.chinese.virus.manageVirus())
  }

  def updateBefore(id: Int) = Action.async { implicit request =>
    virusdao.getById(id).map { x =>
      Ok(views.html.chinese.virus.updateVirus(transForm.transVirusRow(x)))
    }
  }
