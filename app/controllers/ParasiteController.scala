...
class ParasiteController @Inject()(parasitedao: parasiteDao) extends Controller {


  def browse = Action {implicit request=>
    Ok(views.html.english.parasite.browse())
  }
  ...
  def moreInfo(id:Int) = Action.async{implicit request=>
    parasitedao.getById(id).map{x=>
      Ok(views.html.english.parasite.moreInfo(transForm.transParasiteRow(x)))
    }
  }

  def searchByTaxBefore = Action{implicit request=>
    Ok(views.html.english.parasite.searchByTax())
  }
  ...
  def uploadBefore = Action{implicit request=>
    Ok(views.html.english.parasite.uploadParasite())
  }
  ...
  def updateBefore(id:Int) = Action.async{implicit request=>
    parasitedao.getById(id).map{x=>
      Ok(views.html.english.parasite.updateParasite(transForm.transParasiteRow(x)))
    }
  }
  ...
  def manageBefore = Action{implicit request=>
    Ok(views.html.english.parasite.manageParasite())
  }
