...
object dealWithFungus {


  def main(args: Array[String]): Unit = {
    val f = FileUtils.readFileToString(new File("D:\\复旦病原菌数据库资料\\致病菌数据信息 2018.10.12\\数据信息 2018.10.12/Fungus.txt"), "UTF-8")
   ...
   //   val other = Array("亚种（Subspecies）","亚科（Subfamily）","亚目（Suborder）","Superfamily")
    ...
    //最后手动添加多余信息
    FileUtils.writeLines(new File("D:\\复旦病原菌数据库资料\\致病菌数据信息 2018.10.12\\数据信息 2018.10.12/Fungus1.txt"), "UTF-8",(head +: pa).asJava)
  }
}
