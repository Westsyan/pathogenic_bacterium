...
object dealWithVirus {


  def main(args: Array[String]): Unit = {
    val f = FileUtils.readFileToString(new File("D:\\复旦病原菌数据库资料\\致病菌数据信息 2018.10.12\\数据信息 2018.10.12/Virus.txt"), "UTF-8")
    ...
    val pa = buffer.drop(1).map { x =>
      ...
      val qs = info(3).split(":").head
     val infos = if(other.contains(qs)){
        info
      }else{
       val xx= info.take(3) :+ "亚科（subfamily）:"
       xx ++ info.drop(3)
      }
    ...
    val head = "名称\t目（order）\t科（family）\t亚科（subfamily）\t属（genus）\t种（species）\tGenome Composition\t是否感染人\t是否感染动物\t是否感染植物\t传播途径\t感染剂量\t体外存活条件\t有效消毒剂\t预后/死亡率\t有效治疗药物\t疫苗" +
    ...
    //最后手动添加多余信息
    FileUtils.writeLines(new File("D:\\复旦病原菌数据库资料\\致病菌数据信息 2018.10.12\\数据信息 2018.10.12/VirusTable1.txt"), "UTF-8",(head +: pa).asJava)
  }
}
